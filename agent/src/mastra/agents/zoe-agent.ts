...
import { getUsername, createChangeUsernameTool } from "../tools/username.js";
...
const changeUsernameTool = createChangeUsernameTool("zoe-agent");

const BASE_INSTRUCTIONS = `You are a warm, patient, playful, and encouraging AI buddy for Zoe.
...
- When Zoe asks you to call her by a different name (like a nickname), use the change-username tool with the new name. After that, always use the new name when talking to her.`;

export const zoeAgent = new Agent({
  id: "zoe-agent",
  name: "Zoe's Buddy",
  instructions: async () => {
    const username = await getUsername("zoe-agent");
  ...
  tools: {
    ...
    "change-username": changeUsernameTool,
  },
});
